...
sub format_output {
    ...
    if ( $status eq "connecting" ) {
        ...
        $self->print_status_line( $status, $message, $timestamp );
        print "  Found: ",
          colored( $data->{total_plugins} . " plugins", "bright_white" ), "\n";
    }
        ...
        foreach my $update ( @{ $data->{updates} } ) {
            $self->print_progress( $update->{plugin_name},
        ...
        my $success_rate = sprintf( "%.1f%%",
            ( $data->{successful_plugins} / $data->{total_plugins} ) * 100 );
        print "\n", colored( "SEARCH COMPLETED", "green" ), " in ",
          colored( $data->{total_time}, "bright_white" ), "\n";
        print "  Successful plugins: ",
          colored(
"$data->{successful_plugins}/$data->{total_plugins} ($success_rate)",
            "bright_white"
          ),
          "\n";
        print "  Credits remaining: ",
          colored( $data->{creditsLeft}, "bright_white" ), "\n";
    }
...
sub print_result {
                ...
                foreach my $section ( sort keys %$record ) {
                    if ( ref $record->{$section} eq 'HASH' ) {
                        ...
                        foreach my $key ( sort keys %{ $record->{$section} } ) {
                            next
                              if $key =~
                              /^_/;    # skip fields starting with underscore
    ...
    # standard fields
        ...
        if ( $data->{meta} && $data->{meta}->{name} ) {
            print "  Source: ",
              colored( $data->{meta}->{name}, "bright_cyan" ), "\n";
        }

        if ( $data->{badges} ) {
            print "  Badges: ",
              colored( join( ", ", @{ $data->{badges} } ), "yellow" ), "\n";
        }
...
sub print_table {
    ...
    if (   $table->{table_names}
        && ref $table->{headers} eq 'ARRAY'
        && ref $table->{headers}[0] eq 'ARRAY' )
    {
        ...
        for my $table_idx ( 0 .. $#table_names ) {
            ...
            my @values =
              ref $value_sets[$table_idx] eq 'ARRAY'
              ? @{ $value_sets[$table_idx] }
              : ();
                ...
                for my $i ( 0 .. $#headers ) {
                    ...
                    if ( defined $cell ) {
                        if ( ref $cell eq 'ARRAY' ) {
                            print "    ",
                              colored( $header . ":", "bright_cyan" ), "\n";
                            foreach my $item (@$cell) {
                                print "      ",
                                  colored( $item, "bright_white" ), "\n";
                            }
                        }
        ...
        foreach my $row_idx ( 0 .. $#values ) {
            ...
            print "    ",
              colored( "Record #" . ( $row_idx + 1 ), "bright_white" ), "\n";
            print "    ", colored( "-" x 40, "bright_black" ), "\n";
            ...
            for my $i ( 0 .. $#headers ) {
                ...
                if ( ref $cell eq 'HASH' ) {
                    ...
                    foreach my $k ( sort keys %$cell ) {
                        next
                          if $k =~ /^_/;  # skip fields starting with underscore
                        if ( defined $cell->{$k} && $cell->{$k} ne "" ) {
                            print "      $k: ",
                              colored( $cell->{$k}, "bright_white" ), "\n";
                        }
