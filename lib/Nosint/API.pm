...
sub search {
    ...
    my $url =
"https://nosint.org/api/stream-search?target=$target&plugin_type=$plugin_type";

    if ( $self->{aggressive} ) {
        $url .= "&report=true";
        $self->{formatter}->print_info(
            "Aggressive search mode enabled - this will alert the target");
    }
    ...
    $self->{formatter}->print_info(
        "Starting search for $target with plugin type: $plugin_type");

    my $response;

    eval {
        $response = $self->{ua}->request(
            $req,
            sub {
